...
func (r *Reconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	// Reconcile CRDs.
	{
		...
		for _, crd := range []*apiextensionsv1.CustomResourceDefinition{
			...
			op, err = r.CreateOrUpdate(ctx, r.GuestClusterClient, crd, func() error {
				return nil
			})
			if err != nil {
	...
	// Inject userData so we keep the ignition authorization token up todate in the ec2Class.
	{
		...
		for _, ec2NodeClass := range ec2NodeClassList.Items {
			...
			op, err := r.CreateOrUpdate(ctx, r.GuestClusterClient, &ec2NodeClass, func() error {
				...
				ec2NodeClass.Object["spec"].(map[string]interface{})["amiSelectorTerms"] = []map[string]interface{}{
					{
						"id": string(userDataSecret.Labels["hypershift.openshift.io/ami"]),
					},
				}
				return nil
			})
