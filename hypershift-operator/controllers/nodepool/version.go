...
func (r *NodePoolReconciler) nodeVersionsFromMachines(ctx context.Context, machines []*capiv1.Machine, nodePool *hyperv1.NodePool) []hyperv1.NodeVersion {
	...
	for _, machine := range machines {
		...
		// 2. Userdata Secret annotation
		// 3. Fallback to nodePool.Status.Version
		ocpVersion := machine.Annotations[hyperv1.NodePoolReleaseVersionAnnotation]
		if ocpVersion == "" && machine.Spec.Bootstrap.DataSecretName != nil {
			secret := &corev1.Secret{}
			err := r.Get(ctx, crclient.ObjectKey{
				Namespace: machine.Namespace,
				Name:      *machine.Spec.Bootstrap.DataSecretName,
			}, secret)
			if err != nil {
				log.V(4).Info("Failed to get userdata Secret for OCP version lookup", "secret", *machine.Spec.Bootstrap.DataSecretName, "error", err)
			} else {
				ocpVersion = secret.Annotations[hyperv1.NodePoolReleaseVersionAnnotation]
			}
		}
		if ocpVersion == "" {
