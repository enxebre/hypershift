...
func gcpMachineTemplateSpec(
	...
	// Resolve image
	image, err := resolveGCPImage(nodePool, releaseImage, osStream)
	if err != nil {
...
func resolveGCPImage(nodePool *hyperv1.NodePool, releaseImage *releaseinfo.ReleaseImage, osStream string) (string, error) {
	gcpPlatform := nodePool.Spec.Platform.GCP
	...
	image, err := defaultNodePoolGCPImage(nodePool.Spec.Arch, releaseImage, osStream)
	if err != nil {
