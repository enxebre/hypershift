...
func createRotationalEvent(srv *calendar.Service, calendarId string, summary string, startDate, memberEndDate time.Time, recurrenceRule, colorID string) {
	event := &calendar.Event{
		...
		End: &calendar.EventDateTime{
			DateTime: memberEndDate.Format(time.RFC3339),
			TimeZone: "UTC",
		},
...
You should return:
	  -t Cesar,Seth -s 2021-07-01 -d 3 -n SRE-ROLE

E.g if I tell you "Create and event called Interrupt-catcher for Mulham, Juan and Bryan that repeats every 1 week starting the first of july"
You should return:
	  -t Mulham,Juan,Bryan -s 2021-07-01 -d 1 -n Interrupt-catcher

Make sure to return only strictly necessary flags and values format as shown in the examples above.
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if prompt != "" {
				...
				// outputByte := []byte("-t Bryan,Patryk -s 2021-07-05 -d 2 -n snyk-review")
				// Sanitize llm output.
...
func createEvent(ctx context.Context, teamMembers []string, startDate time.Time, weeks int, eventName string) {
	...
	for i, member := range teamMembers {
		...
		memberEndDate := memberStartDate.AddDate(0, 0, durationInDays-1)
		log.Printf("Creating event for %s starting on %v\n", member, memberStartDate)
		iStr := strconv.Itoa(i + 1)
		createRotationalEvent(srv, calendarId, fmt.Sprintf("%s: %s", eventName, member), memberStartDate, memberEndDate, recurrenceRule, iStr)
	}
}
