...
func (p *RegistryClientProvider) Lookup(ctx context.Context, image string, pullSecret []byte) (releaseImage *ReleaseImage, err error) {
	...
	metadataResult, err := DeserializeImageMetadataMultiStream(fileContents[ReleaseImageMetadataFile])
	if err != nil {
		return nil, err
	}
	...
	return &ReleaseImage{
		...
		StreamMetadata:  metadataResult.Default,
		StreamsMetadata: metadataResult.Streams,
	}, nil
}
